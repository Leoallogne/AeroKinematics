...
accounting for aerodynamic drag, wind velocity (X and Y components), and gravity.
"""
...
def _calculate_derivatives(vx: float, vy: float, mass: float, k: float, wind_x: float, wind_y: float, g: float):
    """
    Computes accelerations ax and ay given current velocity, mass, drag factor, and wind vector.
    Equations:
      v_rel_x = vx - wind_x
      v_rel_y = vy - wind_y
      v_rel = sqrt(v_rel_x^2 + v_rel_y^2)
      ax = -(k / mass) * v_rel * v_rel_x
      ay = -g - (k / mass) * v_rel * v_rel_y
    """
    v_rel_x = vx - wind_x
    v_rel_y = vy - wind_y
...
def _interpolate_landing(
    ...
    Formula:
      ...
      x_final = x_current + fraction * (x_next - x_current)
      t_final = t_current + fraction * dt
    """
    if y_curr == y_prev:
        fraction = 0.0
    else:
...
def _simulate_ideal(v0: float, angle_rad: float, mass: float, g: float, dt: float, max_steps: int = 100_000):
    ...
    # Exact final impact point at t = t_flight
...
def _simulate_euler(
    v0: float, angle_rad: float, mass: float, k: float, wind_x: float, wind_y: float, g: float, dt: float, max_steps: int = 100_000
):
    """
    ...
    for _ in range(max_steps):
        ax, ay = _calculate_derivatives(vx, vy, mass, k, wind_x, wind_y, g)
...
def _simulate_rk4(
    ...
    def derivatives(state):
        _x, _y, _vx, _vy = state
        ax, ay = _calculate_derivatives(_vx, _vy, mass, k, wind_x, wind_y, g)
...
def simulate_projectile(
    ...
    euler_result = _simulate_euler(v0=v0, angle_rad=angle_rad, mass=mass, k=k, wind_x=wind_x, wind_y=wind_y, g=g, dt=dt)
    rk4_result = _simulate_rk4(v0=v0, angle_rad=angle_rad, mass=mass, k=k, wind_x=wind_x, wind_y=wind_y, g=g, dt=dt)
